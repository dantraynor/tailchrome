...
describe("ProxyManager", () => {
  ...
  describe("CIDR parsing", () => {
    // We test CIDR parsing indirectly through PAC script output

    it("handles /0 (all traffic)", () => {
      const state = baseState({
        peers: [
          makePeer({ subnets: ["0.0.0.0/0"] }),
        ],
      });
      const pac = capturePAC(pm, state)!;
    ...
    it("handles /32 (single host)", () => {
      const state = baseState({
        peers: [
          makePeer({ subnets: ["10.0.0.5/32"] }),
        ],
      });
      const pac = capturePAC(pm, state)!;
      expect(pac).toContain('isInNet(host, "10.0.0.5", "255.255.255.255")');
    });

    it("handles /8", () => {
      const state = baseState({
        peers: [
          makePeer({ subnets: ["172.0.0.0/8"] }),
        ],
      });
      const pac = capturePAC(pm, state)!;
      expect(pac).toContain('isInNet(host, "172.0.0.0", "255.0.0.0")');
    });

    it("skips invalid CIDR notation", () => {
      const state = baseState({
        peers: [
          makePeer({ subnets: ["not-a-cidr", "10.0.0.0/24"] }),
        ],
      });
      const pac = capturePAC(pm, state)!;
      expect(pac).not.toContain("not-a-cidr");
      expect(pac).toContain('isInNet(host, "10.0.0.0", "255.255.255.0")');
    });
    ...
    it("skips CIDR with prefix > 32", () => {
      const state = baseState({
        peers: [
          makePeer({ subnets: ["10.0.0.0/33"] }),
        ],
      });
      const pac = capturePAC(pm, state)!;
      expect(pac).toContain("// No subnet routes");
    });

    it("skips CIDR with negative prefix", () => {
      const state = baseState({
        peers: [
          makePeer({ subnets: ["10.0.0.0/-1"] }),
        ],
      });
      const pac = capturePAC(pm, state)!;
      expect(pac).toContain("// No subnet routes");
