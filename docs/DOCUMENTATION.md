...
- **Linux ARM64:** download `tailchrome-install.sh` from the exact versioned release, inspect it, download `SHA256SUMS.txt`, and run `bash ~/Downloads/tailchrome-install.sh --version "vX.Y.Z"`. The script selects and verifies `tailscale-browser-ext-linux-arm64`, optionally verifies its GitHub attestation when `gh` is available, invokes `-install-now`, and confirms the installed executable. Never pipe a remote script directly into a shell.
